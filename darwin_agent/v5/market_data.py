...
class MarketDataLayer:
    ...
    def fetch_candles(
        ...
        Uses cache if data is fresh enough. Lock only protects cache dict
        access, NOT the HTTP call — avoids serializing all symbol fetches.
        """
        cache_key = (symbol, interval)
        now = time.time()

        # Check cache (lock only for dict read)
        with self._lock:
        ...
        # HTTP call OUTSIDE the lock — allows parallel fetches per symbol
        try:
            ...
            # Update cache (lock only for dict write)
            with self._lock:
