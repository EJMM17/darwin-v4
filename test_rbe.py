...
_check("init_bar_zero", rbe.bar == 0)
_check("init_mult_one", rbe.last_mult == 1.0)
_check("init_peak_zero", rbe.peak == 0.0)
_check("init_dd_zero", rbe.current_drawdown == 0.0)
...
for i in range(50):
    ...
    m = rbe4.step()
_check("drawdown_reduces_mult", m < 0.80)
# At 100 equity, DD = (150-100)/150 = 33.3%
_check("drawdown_correct", abs(rbe4.current_drawdown - (50.0/150.0)) < 0.01)
...
for i in range(40):
    ...
    m = rbe5.step()
_check("extreme_dd_floor", m <= 0.40)
...
_check("high_vol_reduces", m < 0.95)
...
for i in range(60):
    ...
    rbe7.step()
snap = rbe7.history[-1]
_check("good_pf_positive_bonus", snap.pf_bonus > 0.0)
...
_check("bad_pf_negative_bonus", snap8.pf_bonus < 0.0)
...
_check("bounds_always_respected", all_in_bounds)
...
m1, h1 = run_det()
m2, h2 = run_det()
_check("determinism_mult", abs(m1 - m2) < 1e-12)
_check("determinism_history", all(abs(a - b) < 1e-12 for a, b in zip(h1, h2)))
...
_check("full_stack_calc", abs(size - expected) < 1e-10)
_check("full_stack_nonneg", RiskBudgetEngine.apply_full_stack(0, 0.02, 0.4, 0.75, 0.8) == 0.0)
...
_check("peak_tracks_max", rbe12.peak == 120.0)
_check("dd_after_decline", abs(rbe12.current_drawdown - 10/120) < 1e-6)
...
_check("recovery_restores", m_recov > m_crash)
...
for i in range(10):
    ...
    rbe14.step()
s = rbe14.history[-1]
_check("snap_has_equity", s.equity > 0)
_check("snap_has_peak", s.peak > 0)
_check("snap_has_bar", s.bar == 10)
_check("snap_has_mult", 0.3 <= s.rbe_mult <= 1.0)
...
for i in range(10):
    ...
    rbe15.step()
_check("no_pnl_default_pf", rbe15.history[-1].rolling_pf == 1.5)
